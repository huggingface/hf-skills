...
def get_skill_provenance(skill_dir: Path) -> SkillProvenance:
    ...
    if source is None:
        if error is None:
            return SkillProvenance(status="unmanaged", summary="unmanaged (no sidecar)")
    ...
    if source.source_origin == "remote":
        summary = f"managed (marketplace) • {source.repo_url}{ref_label} • {source.repo_path}"
    else:
        summary = f"managed (local source) • {source.repo_url}{ref_label} • {source.repo_path}"
...
def format_skill_provenance_details(skill_dir: Path) -> tuple[str, str | None]:
    ...
    return managed_value, installed_value
