...
def _github_marketplace_candidates(org: str, repo: str, ref: str, base_path: str) -> list[str]:
    ...
    return [f"https://raw.githubusercontent.com/{org}/{repo}/{ref}/{suffix}" for suffix in suffixes]
...
def load_local_marketplace_payload(url: str) -> Any | None:
    ...
    if parsed.scheme == "file":
        return read_json_file(Path(parsed.path))
...
def derive_local_repo_root(source_url: str) -> str | None:
    ...
    if parsed.scheme in {"http", "https", "ssh"}:
        return None
    path = Path(parsed.path) if parsed.scheme == "file" else Path(source_url)
...
def resolve_source_path_oid(
    *,
    ...
    resolve_local_repo_fn: Any = resolve_local_repo,
    resolve_git_path_oid_fn: Any = resolve_git_path_oid,
) -> tuple[str | None, Any, str | None]:
    cache_key = (repo_url, repo_ref, repo_path, commit)
    resolved: tuple[str | None, Any, str | None]
