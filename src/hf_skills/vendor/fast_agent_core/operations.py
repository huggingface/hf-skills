...
from hf_skills.vendor.fast_agent_core import marketplace_source_utils
from hf_skills.vendor.fast_agent_core.marketplace_parsing import parse_marketplace_payload
from hf_skills.vendor.fast_agent_core.models import (
    LOCAL_REVISION,
...
from hf_skills.vendor.fast_agent_core.provenance import (
...
async def fetch_marketplace_skills_with_source(url: str) -> tuple[list[MarketplaceSkill], str]:
    return await marketplace_source_utils.fetch_marketplace_entries_with_source(
        ...
        parse_payload=lambda payload, source_url: parse_marketplace_payload(payload, source_url=source_url),
    )
...
def select_skill_by_name_or_index(entries: Iterable[MarketplaceSkill], selector: str) -> MarketplaceSkill | None:
...
def select_manifest_by_name_or_index(manifests: Iterable[SkillManifest], selector: str) -> SkillManifest | None:
    selector_clean = selector.strip()
    if not selector_clean:
        return None
...
def select_skill_updates(updates: Sequence[SkillUpdateInfo], selector: str) -> list[SkillUpdateInfo]:
    selector_clean = selector.strip()
    if not selector_clean:
        return []
...
def _check_skill_updates(*, destination_root: Path) -> list[SkillUpdateInfo]:
    ...
    manifests_by_dir = {
        (manifest.path.parent if manifest.path.is_file() else manifest.path): manifest for manifest in manifests
    }
    head_cache: HeadCache = {}
    path_cache: PathCache = {}
    ...
    *,
    ...
    if not manifest_path.exists() or not manifest_path.is_file():
        return SkillUpdateInfo(
            index=index, name=name, skill_dir=skill_dir, status="invalid_local_skill", detail="SKILL.md not found"
        )
    source, error = read_installed_skill_source(skill_dir)
    if source is None:
        if error is None:
            return SkillUpdateInfo(
                index=index, name=name, skill_dir=skill_dir, status="unmanaged", detail="no sidecar metadata"
            )
        return SkillUpdateInfo(index=index, name=name, skill_dir=skill_dir, status="invalid_metadata", detail=error)
    ...
    available_path_oid, path_status, path_error = _resolve_source_path_oid(source, available_revision, path_cache)
    ...
    if current_path_oid is None and source.installed_commit is not None:
        current_path_oid, _, _ = _resolve_source_path_oid(source, source.installed_commit, path_cache)
...
def resolve_source_revision(source: InstalledSkillSource, head_cache: HeadCache) -> HeadResolution:
    cache_key = (source.repo_url, source.repo_ref)
    resolved: HeadResolution
    ...
    if local_repo is not None:
        ...
        head_cache[cache_key] = resolved
        return resolved
    ls_remote_args = ["git", "ls-remote", source.repo_url, source.repo_ref or "HEAD"]
...
def _resolve_source_path_oid(source: InstalledSkillSource, commit: str, path_cache: PathCache) -> PathResolution:
...
def _reinstall_skill_from_source(
    *, skill_dir: Path, source: InstalledSkillSource, revision: str | None
) -> InstalledSkillSource:
    skill_dir = skill_dir.resolve()
    ...
    with tempfile.TemporaryDirectory(dir=parent_dir, prefix=f".{skill_dir.name}.update-") as temp_dir_str:
...
def _copy_skill_from_marketplace_source(
    ...
    *,
    ...
    if local_repo is not None:
        ...
        path_oid = _resolve_git_path_oid(local_repo, commit, skill.repo_path) if commit is not None else None
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        path_oid = _resolve_git_path_oid(tmp_path, commit, skill.repo_path) if commit is not None else None
...
def _resolve_skill_source_dir(source_dir: Path, skill_name: str | None) -> Path:
    if (source_dir / "SKILL.md").exists() or (source_dir.is_file() and source_dir.name.lower() == "skill.md"):
        return source_dir
    ...
    if skills_dir.is_dir():
        candidates = [entry for entry in skills_dir.iterdir() if entry.is_dir() and (entry / "SKILL.md").exists()]
        if len(candidates) == 1:
            return candidates[0]
        if candidates:
            raise FileNotFoundError("Multiple skills found; specify plugins[].skills to select one.")
    return source_dir
